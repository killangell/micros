...
echo "systemctl enable preload.service" >> /$MOUNT_DIR/isodev/firstboot/systemctl_enable.sh

echo "[Unit]" >> /$MOUNT_DIR/isodev/firstboot/preload.service
echo "Description=Preload execute command" >> /$MOUNT_DIR/isodev/firstboot/preload.service
echo "[Service]" >> /$MOUNT_DIR/isodev/firstboot/preload.service
echo "ExecStart=/bin/sh /etc/rc.d/rc.local" >> /$MOUNT_DIR/isodev/firstboot/preload.service
echo "[Install]" >> /$MOUNT_DIR/isodev/firstboot/preload.service
echo "WantedBy=multi-user.target" >> /$MOUNT_DIR/isodev/firstboot/preload.service
...
echo "cp /mnt/sysimage/root/firstboot/preload.service /mnt/sysimage/etc/systemd/system/" >> firstboot-ks-segment.post
echo "chmod 777 /mnt/sysimage/root/firstboot/systemctl_enable.sh" >> firstboot-ks-segment.post
echo "chroot /mnt/sysimage /root/firstboot/systemctl_enable.sh" >> firstboot-ks-segment.post
